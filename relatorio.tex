...
\begin{document}
	...
	O segundo grupo de restrições, as \enquote{convenientes} tornam o problema mais fácil de ser resolvido e eventualmente podemos obter a solução ótima para o problema original através da relaxação ou, se não for o caso, limitantes superior e inferior para fortalecer o problema. \todo{Conferir. Inserir mais alguma coisa?}
	...
	\begin{algoritmo}[h]
        \begin{algorithmic}[1] 
            ...
            \State \indent \indent \indent $ \text{soma} = \text{soma} + x_{val}_j $
        ...
        \end{algorithmic}
    \end{algoritmo}
	\todo{Ajeitar divisão do algoritmo}
	...
	É feita a ressalva de que os tempos de processamento apresentados foram obtidos para uma rodada de cada algoritmo em máquinas próprias. Portanto, os mesmos não incluem incertezas e estão explicitados apenas para fins de comparação entre diferentes métodos para uma mesma instância.\todo{Conferir}
    ...
    Não foi possível \todo{Escrever \enquote{não foi possível}?? Se a gente quisesse mesmo....}
